...
Se usa el comando `table` para describir la nueva variable.
...
De acuerdo al cuestionario, que se puede ver [aquí](https://www.vanderbilt.edu/lapop/ab2018/AB2018-v12.0-Spa-190131_W.pdf), Honduras es el país 4 y Uruguay es el país 14 de la variable "pais".
...
Se incluye una comando lógico "\|" que indica el operador "o".
...
Estos resultados son porcentajes (entre 0 y 100), pero incluye muchos decimales.
...
Esto es: Variable reescalada = ((variable original -1)/6)\*100 El siguiente código usa el operador `<-` que sirve para asignar el resultado de la operación de reescalamiento en una nueva variable del dataframe "lapop18".
...
Apoyo al sistema = (b1rec + b2rec + b3rec + b4rec + b6rec)/5 En R tenemos el comando `rowMeans` que sirva para calcular promedios de ciertas columnas por cada fila.
...
Estos resultados se pueden ver en el Gráfico 3.1 del reporte en gráficos circulares que se verán en la sección siguiente.
