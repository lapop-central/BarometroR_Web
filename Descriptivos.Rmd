...
En primer lugar, veremos aspectos básicos de cómo describir una variable mediante una tabla de distribución de frecuencias y cómo graficar esa variable.
...
Con los datos del Barómetro de las Américas se puede saber el % de hogares con acceso a celulares, con acceso a internet, así como el % de personas que usa Whatsapp, Facebook o Twitter.
...
Estas preguntas tienen como opciones:
...
Estas variables se tienen que convertir a variables de tipo "factor", pues son variables categóricas, las que guardamos en una nueva variable.
...
Como vimos en un documento anterior, se puede calcular nuevas variables con valores condicionales de otras variables usando el comando `ifelse`.
De esta manera, se crea la variable de usuarios de redes sociales.
...
Este comando `table` nos brinda las frecuencias absolutas (número de observaciones) por cada categoría de la variable (en este caso Sí y No).
...
Sin embargo, el comando `prop.table` nos devuelve demasiados decimales.
...
Es mejor construir una sola tabla.
Se puede guardar las tablas parciales en nuevos objetos con el operador `<-` y luego unirlas con el comando `rbind` en un nuevo dataframe "tabla", de tal manera que las respuestas a cada red social aparezcan en filas.
...
# Describir las variables tomando en cuenta en factor de expansión
...
Para reproducir los datos del reporte exactamente se tiene que incluir este efecto de diseño en R mediante el paquete `survey`.
Como esta es una introducción a la estadística descriptiva, no se ha hecho, pero más adelante se incluirá en los cálculos el efecto de diseño.
En este documento se puede usar el comando `freq` que permite la inclusión de una variable de factor de expansión, como "weight1500".
...
En este ejemplo, se ha usado el comando `subset` nuevamente, pero dentro de `ggplot` para que el comando (internamente) trabaje con la variable pero sin los valores perdidos.
...
Luego, la especificación `aes` sirve para definir la "estética" del gráfico.
Generalmente se usa para indicar qué variable se va a graficar y en qué eje (x o y).
...
No existe una geometría directa para hacer un gráfico circulas, por lo que se tiene que usar inicialmente un gráfico de barras simple, usando el comando `geom_bar()`, donde internamente se define el ancho de la barra.
Si dejáramos la sintaxis en este punto, se generaria una barra que se dividiría entre los valores de la variable "smedia1r".
...
Un aspecto a resaltar es que en este caso se está usando el pipe `%>%` de la librería `dply`, que es una forma (un poco) diferente de escribir códigos en R, de manera concatenada, paso a paso.
...
La especificación `lab.pos` no la explicaremos aquí (aunque sirve para luego ubicar la etiqueta en el gráfico).
...
Nótese que, en este caso, los datos que se usan vienen del dataframe df (no lapop18).
...
En este caso tambien se puede usar los datos agrupados del dataframe "df".
...
En todos estos cálculos no se ha tomado en cuenta el efecto de diseño, ni se ha incorporado el factor de expansión en los cálculos.
Para comparar ambos resultados, sin y con la incorporación del efecto de diseño, se puede usar el comando `freq` sin y con la variable "weight1500".
...
Sin considerar el efecto de diseño, se tiene que 57.08% de entrevistados cuenta con una cuenta de Facebook.
Este porcentaje varía a 57.15% si se incluye la variable de expansión.
