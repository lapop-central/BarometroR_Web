...
Ahí se tendrá acceso a la base de datos completa "2018 LAPOP AmericasBarometer Merge_v1.0_W.dta" en versión para STATA.
...
Estas variables se tienen que convertir a variables de tipo "factor", pues son variables categóricas, las que guardamos en una nueva variable "smediaxr".
Aquí se ha usado el operador `=` que es similar al operado `<-` que asigna un procedimiento a un nuevo objeto de (un dataframe) R
...
Estas nuevas variables de tipo factor se tienen que etiquetar.
En este caso se hace mediante un vector con las etiquetas concatenadas, usando el comando `c()`.
...
En la página 54 del reporte se observa que se reportan estos datos en forma gráfica, mediante un gráfico de sectores circulares.
