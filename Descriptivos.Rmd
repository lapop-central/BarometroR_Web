...
En la página 54 del reporte se observa que se reportan estos datos mediante un gráfico de sectores circulares.
...
Esta tabla incluye datos que no requerimos, como la fila de NA´s y del total.
Estas filas se borran usando la especificación `[-c(3,4), ]`.
Si se quisiera borrar la primera columna se tendría que escribir `[, -c(1)]`.

Luego, se le cambia el nombre a las columnas para evitar el nombre "Valid Percent".
Se las nombre simplemente como "freq", "per" y "vper".
Esta última columna es la que nos sirve.
...
Nótese que ahora se usa el dataframe "fb" y que en aes se especifica que en el eje X deben estar los datos de la columna "lab" y en el eje Y los datos de la columna "vper".
